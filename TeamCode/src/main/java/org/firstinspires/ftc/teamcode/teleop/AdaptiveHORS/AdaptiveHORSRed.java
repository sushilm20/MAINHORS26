 ...
 *
 * <p><b>FIX:</b> Blue-side defaults are explicitly restored before mirroring,
...
public class AdaptiveHORSRed extends LinearOpMode {
    ...
    // FIX: Initialize to RED_START_POSE so first-frame RPM calc uses real position
    ...
    /**
     * Restores all ShooterCalibration statics to their blue-side defaults.
     * Called at the start of runOpMode (to undo a previous Red run) and
     * in the finally block (to leave clean state for a subsequent Blue run).
     */
    private void restoreBlueDefaults() {
    ...
    public void runOpMode() {
        ...
        // ════════════ Flywheel + Mirrored Adaptive RPM ════════════
        flywheel = new FlywheelController(shooter, shooter2, telemetry, batterySensor);
        flywheel.setShooterOn(false);

        // FIX: First restore blue defaults (in case a previous Red run left
        //      them dirty), THEN mirror for red. Wrap the entire OpMode in
        //      try/finally so we always restore blue defaults on exit.
        restoreBlueDefaults();

        try {
            // Mirror the blue goal to get the red goal
            Pose redGoalMirrored = mirrorPose(DEFAULT_GOAL_X, DEFAULT_GOAL_Y);
            ...
            Pose m1 = mirrorPose(DEFAULT_CAL1_X, DEFAULT_CAL1_Y);
            ShooterCalibration.CAL1_X = m1.getX(); ShooterCalibration.CAL1_Y = m1.getY();

            Pose m2 = mirrorPose(DEFAULT_CAL2_X, DEFAULT_CAL2_Y);
            ShooterCalibration.CAL2_X = m2.getX(); ShooterCalibration.CAL2_Y = m2.getY();

            Pose m3 = mirrorPose(DEFAULT_CAL3_X, DEFAULT_CAL3_Y);
            ShooterCalibration.CAL3_X = m3.getX(); ShooterCalibration.CAL3_Y = m3.getY();

            Pose m4 = mirrorPose(DEFAULT_CAL4_X, DEFAULT_CAL4_Y);
            ShooterCalibration.CAL4_X = m4.getX(); ShooterCalibration.CAL4_Y = m4.getY();

            Pose m5 = mirrorPose(DEFAULT_CAL5_X, DEFAULT_CAL5_Y);
            ShooterCalibration.CAL5_X = m5.getX(); ShooterCalibration.CAL5_Y = m5.getY();

            Pose m6 = mirrorPose(DEFAULT_CAL6_X, DEFAULT_CAL6_Y);
            ShooterCalibration.CAL6_X = m6.getX(); ShooterCalibration.CAL6_Y = m6.getY();

            Pose m7 = mirrorPose(DEFAULT_CAL7_X, DEFAULT_CAL7_Y);
            ...
            // Red goal pose for FlywheelVersatile (passed for API compat;
            // distance is now computed inside ShooterCalibration.distanceToGoal)
            ...
            // FIX: Use calibration's distanceToGoal so distance uses the same
            //      (now-mirrored) GOAL_X / GOAL_Y as the regression.
            ...
            // FIX: Always restore blue-side defaults so the next OpMode
            //      (Blue or Red) starts from clean canonical values.
            restoreBlueDefaults();
        }
    }
