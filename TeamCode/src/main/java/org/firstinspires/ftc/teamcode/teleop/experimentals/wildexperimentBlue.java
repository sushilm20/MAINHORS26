...
public class wildexperimentBlue extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        // Distance tracker - this does the auto RPM/hood calculation
        ...
        while (opModeIsActive()) {
            ...
            // Update follower and distance tracker
            if (follower != null) {
                follower.update();
            }
            distanceTracker.update(follower);

            // ===== RESET (A button or GP2 touchpad) =====
            ...
            // ===== TOGGLE AUTO MODE (touchpad or RB) =====
            boolean touchpadNow = getTouchpad(gamepad1) || gamepad2.right_bumper;
            if (touchpadNow && !touchpadPressedLast) {
                autoModeEnabled = !autoModeEnabled;
                if (!autoModeEnabled) {
                    // Switch to manual - keep current values
                    flywheel.setTargetRPM(distanceTracker.getRpm() + rpmTrim);
                }
            }
            ...
            // ===== FLYWHEEL CONTROLS =====
            if ((gamepad1.dpad_down || gamepad2.dpad_down) && !dpadDownLast) {
            ...
            // Hood trim (GP2 left stick)
            if (gamepad2.left_stick_y < -0.5) {
                hoodTrim += CalibrationPoints.HOOD_TRIM_STEP;
            } else if (gamepad2.left_stick_y > 0.5) {
                hoodTrim -= CalibrationPoints.HOOD_TRIM_STEP;
            }

            // ===== GATE =====
            if ((gamepad1.b || gamepad2.b) && !bPressedLast && !gateController.isBusy()) {
                gateController.toggleGate();
            }
            bPressedLast = gamepad1.b || gamepad2.b;

            // ===== INTAKE SEQUENCE =====
            if ((gamepad1.y || gamepad2.y) && !yPressedLast && !gateController.isBusy()) {
                gateController.startIntakeSequence(nowMs);
            }
            yPressedLast = gamepad1.y || gamepad2.y;

            if (gateController.update(nowMs)) {
            ...
            telemetry.addLine();
            telemetry.addData("RPM", "%.0f / %.0f (trim: %.0f)",
                    flywheel.getCurrentRPM(), flywheel.getTargetRPM(), rpmTrim);
            telemetry.addData("Hood", "%.3f (trim: %.3f)",
                    hoodController.getRightPos(), hoodTrim);

            // Debug: show raw follower pose
            if (follower != null) {
